...
where $\epsilon = \frac{\Delta t}{2}$.  By discretizing in the finite element space $H_0(\divv) \times L_0^2$, as seen in \eqref{FEcoeffoperator}, we recover our finite dimensional coefficient operator, $\mathscr{A}_h$, defined as
...
\end{bmatrix}
\end{equation}
%This is a standard preconditioning approach utilizing the Riesz map derived from the problem's spaces. 
%Need Brezzi conditions here with inf sup condition possibly
%Should I provide analysis on A here?
%schur complement - 0517 notes
%A is elliptic
...
%The Riesz map gets rid of mesh dependence but not parameter dependence??. [m/w] Want to adjust B so it is still easy to invert but the bounds on B^-1A are as independent of eps as we can make them. 
