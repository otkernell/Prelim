...
\begin{frame}{Preconditioning}
	\begin{block}{Discrete Form}
		...
		\end{bmatrix}.
		\end{gather*}
		For this preconditioner, our goal is to look at the eigenvalues of $B_h^{-1}A_h$ as a function of $\alpha$, $\beta$, and $\gamma$. We want to choose these parameters so that our preconditioner is robust in terms of $\Delta t$ and $h$, thus eliminating the dependence on $\epsilon$.
	\end{block}
\end{frame}

\begin{frame}
	\begin{block}{Conjecture 1}
		The preconditioner $\mathscr{B}$ is bounded with bounded inverse
	\end{block}
	\begin{block}{Conjecture 2}
		The condition number $\kappa$ of $\mathscr{B}$ depends on $\Delta t$, $\alpha$, $\beta$, and $\gamma$
	\end{block}
